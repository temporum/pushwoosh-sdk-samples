...
/**
 * TableLayoutManager can be used to create multi column table views. You can
 * even embed table within another table to create complex tabular views.
 */
public class TableLayoutManager extends Manager {
			...
			for (int j = 0; j < rows; j++) {
				...
				if (field != null) {
					int actualHeight = getPreferredHeightOfChild(field)
							+ field.getMarginBottom() + field.getMarginTop();
	...
	protected void sublayout(int layoutWidth, int layoutHeight) {
						...
						if (field != null) {
							layoutChild(
									...
									Math.max(
											0,
											layoutWidth
													- (field.getMarginLeft() + field
															.getMarginRight())),
									Math.max(
											0,
											layoutHeight
													- (field.getMarginBottom() + field
															.getMarginTop())));
		...
		for (int n = 0; n < numberFields; n++) {
			...
			_rowHeights[currentRow] = Math.max(
					_rowHeights[currentRow],
					field.getExtent().height + field.getMarginBottom()
							+ field.getMarginTop());
			...
			if ((n == (numberFields - 1)) || (currentColumn >= _columns)) {
				...
				for (int i = 0; i < currentColumn; i++) {
					...
					XYPoint offset = calcAlignmentOffset(field1, Math.max(
							0,
							_columnWidths[i]
									- (field1.getMarginLeft() + field1
											.getMarginRight())), Math.max(
							0,
							_rowHeights[currentRow]
									- (field1.getMarginBottom() + field1
											.getMarginTop())));
					setPositionChild(field1,
							x + offset.x + field1.getMarginLeft(), y + offset.y
									+ field1.getMarginTop());
	...
	/**
	 * Calculate the styles and return appropriate XY offset locations within
	 * the cell.
	 * 
	 * @param field
	 * @param width
