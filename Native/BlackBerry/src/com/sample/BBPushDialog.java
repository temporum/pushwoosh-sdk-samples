...
import com.arellomobile.blackberry.push.OKButton;
...
public class BBPushDialog extends PopupScreen implements FieldChangeListener {
	...
	private OKButton okButton = new OKButton(ButtonField.CONSUME_CLICK
			| FOCUSABLE | Field.FIELD_BOTTOM | Field.FIELD_HCENTER);

	/**
	 * Creates a custom dialog, a little prettier than the default Blackberry
	 * Alert
	 * 
	 * @param message
	 *            Message to display in alert box
	 */
	public BBPushDialog(String message) {
		super(new VerticalFieldManager(FIELD_VCENTER), Manager.FIELD_VCENTER
				| Manager.FIELD_HCENTER);

		_message = message;

		vfm = new VerticalFieldManager(USE_ALL_WIDTH | Manager.FIELD_VCENTER
				| Manager.FIELD_HCENTER | Manager.VERTICAL_SCROLL
				| Manager.VERTICAL_SCROLLBAR);

		TableLayoutManager hfm = new TableLayoutManager(new int[] {
				TableLayoutManager.FIXED_WIDTH,
				TableLayoutManager.SPLIT_REMAINING_WIDTH }, new int[] {
				chops.getWidth() + 30, 0 }, 0, Manager.USE_ALL_WIDTH
				| FIELD_HCENTER);

		BitmapField chopsField = new BitmapField(chops, Field.FIELD_LEFT
				| Field.FIELD_VCENTER);
