...
- (void)viewDidLoad
{
    ...
    // table
    [table registerNib:[UINib nibWithNibName:@"IssueTableCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:issueTableCellId];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [publisher setCoverOfIssueAtIndex:index completionBlock:^(UIImage *img) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UITableViewCell *cell = [table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:indexPath.row inSection:0]];
            UIImageView *imageView = (UIImageView *)[cell viewWithTag:100];
            imageView.image = img;
        });
    }];
...
-(void)updateProgressOfConnection:(NSURLConnection *)connection withTotalBytesWritten:(long long)totalBytesWritten expectedTotalBytes:(long long)expectedTotalBytes {
    ...
    UITableViewCell *cell = [table cellForRowAtIndexPath:[NSIndexPath indexPathForRow:[[assetDownload.userInfo objectForKey:@"Index"] intValue] inSection:0]];
